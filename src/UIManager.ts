...
import { EFFECT_QUALITY_PRESETS, GameConfig } from './GameConfig.js';
...
export class UIManager {
    ...
    private effectSettingsCallback?: (config: Partial<GameConfig>) => void;
    ...
    public setEffectSettingsCallback(callback: (config: Partial<GameConfig>) => void): void {
        this.effectSettingsCallback = callback;
    }

    private setupOverlaySettings(): void {
    ...
    private updateDifficultyButtonStyles(activeBtn: HTMLElement, ...otherBtns: HTMLElement[]): void {
    ...
    private setupOverlayEffectSettings(): void {
        const qualitySelect = document.getElementById('overlayEffectQuality') as HTMLSelectElement;
        const maxEffectsSlider = document.getElementById('overlayMaxEffects') as HTMLInputElement;
        const maxEffectsValue = document.getElementById('overlayMaxEffectsValue') as HTMLElement;
        const particleDensitySlider = document.getElementById('overlayParticleDensity') as HTMLInputElement;
        const particleDensityValue = document.getElementById('overlayParticleDensityValue') as HTMLElement;
        ...
        qualitySelect.onchange = (e) => {
            e.stopPropagation();
            const quality = qualitySelect.value as 'low' | 'medium' | 'high' | 'ultra';
    ...
    public updateEffectSettings(config: GameConfig): void {
    ...
    public updateDifficultyDisplay(difficultyState: any): void {
        if (!difficultyState) return;
    ...
    private getDifficultyColor(difficultyName: string): string {
        switch (difficultyName.toLowerCase()) {
            case 'beginner': case 'stable': return '#00ff00';
            case 'intermediate': case 'chaotic': return '#ffff00';
            case 'advanced': case 'pandemonium': return '#ff8800';
            case 'expert': case 'apocalypse': return '#ff4400';
