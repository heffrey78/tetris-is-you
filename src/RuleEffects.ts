...
export interface PropertyEffect {
    ...
    applyToPhysics?: (block: TetrisBlock, gameState: GameState) => { shouldFall: boolean; shouldDestroy: boolean };
    visualEffect?: (block: TetrisBlock) => { opacity?: number; glow?: boolean; animation?: string };
}

export class RuleEffects {
    private static readonly PROPERTY_EFFECTS: Record<string, PropertyEffect> = {
        // Destruction Spells
        'BOMB': {
            name: 'BOMB',
    ...
    public static getEffect(property: string): PropertyEffect | null {
        return this.PROPERTY_EFFECTS[property.toUpperCase()] || null;
    }
    
    public static getAllEffects(): PropertyEffect[] {
        return Object.values(this.PROPERTY_EFFECTS);
    }
    
    public static getEffectDescription(property: string): string {
        const effect = this.getEffect(property);
        return effect ? effect.description : `Unknown property: ${property}`;
    }
    
    public static applyRulesToBlock(block: TetrisBlock, activeRules: Rule[], ruleEngine?: RuleEngine): TetrisBlock {
    ...
    public static checkWinConditions(gameState: GameState, activeRules: Rule[]): boolean {
    ...
    public static checkLoseConditions(gameState: GameState, activeRules: Rule[]): boolean {
