...
export class Renderer {
    ...
    drawPlayfieldBorder() {
        ...
        this.ctx.strokeRect(this.playfieldStartX + borderOffset, this.playfieldStartY + borderOffset, this.playfieldWidth - this.ctx.lineWidth, this.playfieldHeight - this.ctx.lineWidth);
    ...
    drawEnhancedBlock(gridX, gridY, block, rules) {
        const gridSize = LAYOUT.SCALED_GRID_SIZE || LAYOUT.GRID_SIZE;
        const pixelX = this.playfieldStartX + (gridX * gridSize);
        const pixelY = this.playfieldStartY + (gridY * gridSize);
    ...
    applyGlowEffect(pixelX, pixelY, gridSize, block, rules) {
        ...
        if (blockRules.includes('BOMB'))
            glowColor = '#ff8800';
        else if (blockRules.includes('HEAL'))
            glowColor = '#00ff44';
        else if (blockRules.includes('SHIELD'))
            glowColor = '#0088ff';
        else if (blockRules.includes('LIGHTNING'))
            glowColor = '#ffff00';
    ...
    drawEnhancedBorder(pixelX, pixelY, gridSize, block, rules, visualEffects) {
        const blockRules = this.getApplicableRuleProperties(block, rules);
        this.ctx.lineWidth = 1;
    ...
    drawPatternOverlay(pixelX, pixelY, gridSize, block, rules, visualEffects) {
        const blockRules = this.getApplicableRuleProperties(block, rules);
