...
export class EffectManager {
    constructor(canvas) {
        ...
        this.gridSize = LAYOUT.SCALED_GRID_SIZE || LAYOUT.GRID_SIZE;
    }
    /**
     ...
     */
    createAcidParticles(effect, pixelPos) {
    ...
    createFlameParticles(effect, pixelPos) {
    ...
    createCrumbleParticles(effect, pixelPos) {
        // Will implement crumbling particles here
    }
    createExplosionParticles(effect, pixelPos) {
        // Will implement explosion particles here
    }
    createFreezeParticles(effect, pixelPos) {
        // Will implement freeze particles here
    }
    createLightningParticles(effect, pixelPos) {
        // Will implement lightning particles here
    }
    /**
     * Add a combo effect (full-screen animation)
     */
    addComboEffect(comboLevel, intensity = 1.0) {
        let effectType;
     ...
     */
    getEffectsAt(gridPosition) {
        return Array.from(this.effects.values()).filter(effect => effect.gridPosition.x === gridPosition.x &&
            effect.gridPosition.y === gridPosition.y);
    }
    /**
     * Check if any effects are active at a position
     */
    hasEffectsAt(gridPosition) {
